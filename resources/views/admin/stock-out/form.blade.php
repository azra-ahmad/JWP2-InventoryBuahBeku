@extends('layouts.admin', ['title' => ($transaction->exists ? 'Edit' : 'Tambah') . ' Stok Keluar - MeyJuice', 'pageTitle' => 'Stok Keluar'])

@section('content')
    @include('partials.page-header', [
        'title' => $transaction->exists ? 'Edit Stok Keluar' : 'Tambah Stok Keluar',
        'subtitle' => 'Sistem menolak stok keluar jika persediaan tidak cukup.',
    ])

    <form method="POST" action="{{ $transaction->exists ? route('admin.stock-out.update', $transaction) : route('admin.stock-out.store') }}" class="max-w-2xl rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        @csrf
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="block">
                <span class="mb-1 block text-sm font-semibold text-slate-700">Tanggal Keluar</span>
                <input type="date" name="tanggal_keluar" value="{{ old('tanggal_keluar', optional($transaction->tanggal_keluar)->format('Y-m-d') ?? now()->format('Y-m-d')) }}" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
            </label>
            <label class="block md:col-span-2">
        ...
        <div class="mt-5 flex gap-2">
            ...
            <a href="{{ route('admin.stock-out.index') }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">Batal</a>
        </div>
    </form>
@endsection
